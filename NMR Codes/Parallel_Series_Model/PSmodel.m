...
%         end
        ...
        T2lm_parallel(jj,1) = (shortSummedSignal(jj)/allPhi(jj) * T2lm_short(jj))+(longSummedSignal(jj)/allPhi(jj) * T2lm_long(jj));
        T2lm_series(jj,1) = 1/(((shortSummedSignal(jj)/allPhi(jj))/T2lm_short(jj)) + ((longSummedSignal(jj)/allPhi(jj))/T2lm_long(jj)));
        ...
        SDR_K_parallel(jj) = SDR_K(allSDRb(jj), allSDRm(jj), allSDRn(jj),...
        allPhi(jj), T2lm_parallel(jj));

        SDR_K_series(jj) = SDR_K(allSDRb(jj), allSDRm(jj), allSDRn(jj),...
        allPhi(jj), T2lm_series(jj));
...
%         end
    ...
    if figureson == 1 && T2_bimodal(jj) == 1
        allK(jj)
        
        %Checked on 6/6 T2ML and T2lm are the same
        [val, index] = min(abs(allT2ML(jj) - T2linbins));
        [val, index2] = min(abs(T2lm_long(jj) - T2linbins));
        [val, index3] = min(abs(T2lm_short(jj) - T2linbins));
...
%         plot(T2linbins(indShortMax(jj,1)), sliceT2(indShortMax(jj,1)), 'g*','MarkerSize',10)
        ...
        legendString = strcat('T_2 for K= ', sprintf('%d',allK(jj)),'m/s');
        %fileString = strcat('K= ', sprintf('%d',allK(jj)), allSites(jj),'.png');

        fileString = strcat('z=',num2str(allZ(jj)),'_','K=',sprintf('%d',allK(jj)),'_', allSites(jj),'.png');
        
        title(strcat('Short Weight=',num2str(shortSummedSignal(jj)),' Long Weight=',...
            num2str(longSummedSignal(jj)),' NMR Phi=',num2str(allPhi(jj))))
        
        legend(legendString,'Long Signal', 'Short Signal', 'T2ML', ...
            'Long T2ML', 'Short T2ML','Location','northwest')
        print('-dpng','-r300',fileString)
        
        ylabel('Amplitude (porosity units)')
        xlabel('Relaxation Time (s)')
    end
    
end

% k_estimates = [Kreg; Kcorr];
% k_estimates = k_estimates';
% k_names = {'Regular SDR, optimal b','Corrected Model, wrong b'};
% plotKestKdpp_v2(allK,k_estimates,k_estimates,k_names)

k_estimates = [KregSDR; Kparallel];
k_estimates = k_estimates';
k_names = {'Regular SDR (m=0, n=2)','K parallel SDR (m=0, n=2)'};
plotKestKdpp_v2(allK,k_estimates,k_estimates,k_names)
plotKFactor_v2(allK,k_estimates,k_estimates,k_names)
diffFactorParallel(1:83,1:2) = estimateKdiffFactor(allK,k_estimates,1);
meddiff_parallel = median(diffFactorParallel, 'OmitNaN')

k_estimates = [KregSDR; Kseries];
k_estimates = k_estimates';
k_names = {'Regular SDR (m=0, n=2)','K series SDR (m=0, n=2)'};
plotKestKdpp_v2(allK,k_estimates,k_estimates,k_names)
plotKFactor_v2(allK,k_estimates,k_estimates,k_names)
diffFactorSeries(1:83,1:2) = estimateKdiffFactor(allK,k_estimates,1);
meddiff_series = median(diffFactorSeries, 'OmitNaN')

k_estimates = [KregSDR; SDR_K_parallel];
k_estimates = k_estimates';
k_names = {'Regular SDR (m=0, n=2)','SDR K T2ml parallel (m=0, n=2)'};
plotKestKdpp_v2(allK,k_estimates,k_estimates,k_names)
plotKFactor_v2(allK,k_estimates,k_estimates,k_names)
diffFactorParallel(1:83,1:2) = estimateKdiffFactor(allK,k_estimates,1);
meddiff_SDRparallel = median(diffFactorParallel, 'OmitNaN')

k_estimates = [KregSDR; SDR_K_series];
k_estimates = k_estimates';
k_names = {'Regular SDR (m=0, n=2)','SDR K T2ml series (m=0, n=2)'};
plotKestKdpp_v2(allK,k_estimates,k_estimates,k_names)
plotKFactor_v2(allK,k_estimates,k_estimates,k_names)
diffFactorSeries(1:83,1:2) = estimateKdiffFactor(allK,k_estimates,1);
meddiff_SDRseries = median(diffFactorSeries, 'OmitNaN')



plotKestKdpp_bimodal(allK,Kreg',T2_bimodal');

figure(10)
scatter(allK, Sand_frac_approx,60,'Filled')

grid on
box on 

xlabel('DPP K (m/s)')
ylabel('Long T2 Mode Sum/Porosity') 
...
scatter(KregSDR,Sand_frac_approx,60,'Filled')
scatter(Kparallel, Sand_frac_approx,60,'Filled')
scatter(Kseries, Sand_frac_approx,60,'Filled')

% scatter(Sand_frac_approx,KregSDR,60,'Filled')
% scatter(Sand_frac_approx,Kparallel,60,'Filled')
% scatter(Sand_frac_approx,Kseries,60,'Filled')

grid on
box on 

xlabel('K estimate (m/s)')
ylabel('(Long T2 Mode Sum)/Porosity') 

legend('Reg SDR K','Parallel K','Series K')
