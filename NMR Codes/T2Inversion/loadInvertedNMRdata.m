function [T2dist,T2linbins,nmrName] = loadInvertedNMRdata(siteName, smooth)
...
%         T2logbins = load(in2)';
    ...
    elseif sum(strcmp(siteName,wisc_sites) == 1)
       ...
       in1 = [rawBaseDir site '/' name '/' name '_' num2str(smooth) '_newInvert.mat']; 
        
       T2data = load(in1);
       T2dist = T2data.newT2Dist;
       T2dist(:,1) = T2dist(:,1) - offset; 
       T2linbins = T2data.newT2linbins;
