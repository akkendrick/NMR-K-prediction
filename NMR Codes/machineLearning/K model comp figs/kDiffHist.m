...
load Seevers_bestFit_0407_m1_n2_T2BMap.mat
sites = siteList;
...
load Seevers_maurer_bestFit_0407_m1_n2_T2BMap.mat
sites = siteList;
Seevers_b = squeeze(totalbMatrix(1,1,:))';
Seevers_n = n;
Seevers_n = repmat(n,length(siteList),1);
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
Temp = 11.111;  % temperature in degress C 
rho = @(Tt) 1000*(1 - ((Tt+288.94)./(508929*(Tt+68.12))).*(Tt-3.98).^2); % kg/m^3
eta = @(Tt) 0.0013 - 1.7e-5*Tt;         % Pa -s
Tb = @(Tt) 3.3 + 0.044*(Tt - 35);       % seconds
D = @(Tt) (1.0413 + 0.039828*Tt + 0.00040318*Tt.^2).*1e-9;  % m^2/s 
...
neghistSDR = histDataSDR(histDataSDR < 0);
...
neghistSeevers = histDataSeevers(histDataSeevers < 0);
...
neghistSOE = histDataSOE(histDataSOE < 0);
...
neghistKGM = histDataKGM(histDataKGM < 0);
