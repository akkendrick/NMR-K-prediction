...
siteList = [{'Site1-WellG6'}...
   {'Site1-WellG5'} {'Site2-WellPN1'} {'Site2-WellPN2'}];
...
elseif isempty(m) && ~isempty(n)
    ...
    for j = 1:length(m)
       ...
       for i = 1:length(siteList)
            ...
            if m(j) == 99
                [K,z,k_boot,k_mcmc,k_direct,bestFitMatrix,totalErrorEstimate] = computeProfile(siteName,n(j),[],figureson,wDirect,saveData);
            else
                [K,z,k_boot,k_mcmc,k_direct,bestFitMatrix,totalErrorEstimate] = computeProfile(siteName,n(j),m(j),figureson,wDirect,saveData);
            end
...
save('SDR_bestFit_1101_m1_n1.mat','siteList','m','n','matrixKey','totalbMatrix','totalmMatrix','totalErrorMatrix')
