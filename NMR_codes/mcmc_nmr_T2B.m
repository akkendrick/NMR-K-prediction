...
bounds = [minlogb, maxlogb; minn, maxn; ...
...
% run mcmc
[paramhats, likes, kpreds, accept_rat] = mcmc(Niter, stepsize, @NMRfunT2B, x_init, k, ...
    bounds, [], [], bcut, [],Dk, phi, T2ML, T2B, ip);

if figureson == 1
    names = {'log_{10}(b)', 'n', 'm', '\sigma'}; 
...
hps = dataset(datamat(:,1), 10.^datamat(:,1), datamat(:,2), datamat(:,3), datamat(:,4), 'VarNames', Varnames, 'ObsNames', obsnames); 
